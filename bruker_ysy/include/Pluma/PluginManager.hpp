...
//
// Changed by ParaVision team, Bruker BioSpin MRI GmbH:
// - Add a load and loadFromFolder functions that do not reload already 
//   existing plugins.
////////////////////////////////////////////////////////////
...
public:
    ...
    /// \see load(const std::string&, bool)
    /// \see loadFromFolder
    /// \see unload
    /// \see unloadAll
    ///
    ...
    /// If file extension isn't present on the path, Pluma will deduce it
    /// from the operating system.
    /// \param forceLoad If \c False the plugin is only loaded if it does not
    /// already exist in the plugin list.
    ///
    ...
    ///
    ////////////////////////////////////////////////////////////
    bool load(const std::string & path, bool forceLoad);
    ...
    /// \param forceLoad If \c False a plugin is only loaded if it does not
    /// already exist in the plugin list.
    /// \param recursive If true it will search on sub-folders as well
    ///
    /// \return Number of successfully loaded or existing plug-ins.
    ///
    /// \see loadFromFolder(const std::string&, bool, bool);
    /// \see load(const std::string&, const std::string&)
    /// \see load(const std::string&)
    /// \see unload
    /// \see unloadAll
    ...
    ///
    /// \see loadFromFolder(const std::string&, bool);
    /// \see load(const std::string&, const std::string&)
    /// \see load(const std::string&)
    /// \see unload
    /// \see unloadAll
    ///
    ////////////////////////////////////////////////////////////
    int loadFromFolder(const std::string& folder, bool forceLoad, bool recursive);
