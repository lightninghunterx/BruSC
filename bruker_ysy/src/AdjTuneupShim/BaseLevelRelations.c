...
void SetBasicParameters( void )
{
  ...
  /* NR */
  int repetitions=PVM_NRepetitions;
  if(TestDynamicShims==Yes && DynShimAcqExperiment == Yes)
  {
    repetitions *= NDynShims;
  }
  ATB_SetNR( repetitions );
...
void SetPpgParameters( void )
{
  ...
  ATB_SetPulprog("AdjTuneupShim.ppg");
...
void printTiming(void)
{
  double aqq=PVM_SpecAcquisitionTime, tr;
   ...
   /* TR */
  tr =0.02+d(0)+p(0)+d(1)+aqq+5.0+0.01+1.0;
