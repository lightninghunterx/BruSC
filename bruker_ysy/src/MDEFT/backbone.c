...
void backbone(void)
{
  ...
  SegmNumber = PVM_EncGenTotalSteps/SegmentSize;
...
void UpdateEchoRepTime(void)
{
  ...
  fixedTime = (EchoRepTime * (NDummyEchoes + PVM_EncCentralStep1-1))+PreEchoTime;

  SegmDuration = EchoRepTime * (SegmentSize + NDummyEchoes);
  ...
  if(PreparationMode == Inversion_prep)
  {
    ...
    MinSegmRepTime = SegmDuration + Mdeft_PrepModuleTime;
  ...
  else
  {
    ...
    SegmRepTime = SegmDuration + Mdeft_PrepModuleTime;
  }
  ...
  if( dim >1 )
    TotalTime = SegmRepTime * PVM_EncGenTotalSteps/SegmentSize;

//  if( dim >2 )
//     TotalTime = TotalTime * PVM_EncGenLoopOuter;
...
void UpdateRFSpoiling(void)
{
  ...
  {
    int max = SegmentSize*(SegmNumber+PVM_DummyScans);
