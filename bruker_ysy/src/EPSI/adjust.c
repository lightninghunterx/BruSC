 ...
 *
 * ***************************************************************/
const char * methods_EPI_adjust = "$Id$ (C) 2006-2016 Bruker BioSpin MRI GmbH ";
...
void HandleAdjustmentRequests(void)
{
  ...
  {
    DB_MSG(("Handle ArrayPhaseAdjustment"));
    PVM_NAverages = 1;
    ParxRelsParRelations("PVM_NAverages",No);
    PVM_EpiPrefixNavYes=Yes;  //use nav signal for phase analysis
...
void setRGGhostAdj(YesNo DoubleShot)
{
  ...
  /* no blips in EPSI -> no effect of segments on EpiRampTime
    -> in opposite to EPI, no FOV reduction necessary here */
