...
void backbone( void )
{
  ...
  DB_MSG(("Entering T2S_EPI/backbone.c:backbone"));
  ...
  {
    ParxRelsHideInEditor("SpinEchoTime, RefPul, RefPulseEnum, RefPulseAmpl");
  }
  else
  {
    ParxRelsShowInEditor("SpinEchoTime, RefPul, RefPulseEnum");
    if (PVM_DeriveGains == Yes)
    {
  ...
  EchoTimeIncrement=MAX_OF(EchoTimeIncrement,PVM_EpiModuleTime + 0.01);
  EchoTimeIncrement=MIN_OF(EchoTimeIncrement,32*PVM_EpiModuleTime);
  ...
  DB_MSG(("Exiting T2S_EPI/backbone.c:backbone"));
}
...
{
  DB_MSG(("Entering T2S_EPI/backbone.c:echoTimeRels()"));

  PARX_change_dims("GradientEchoTime",PVM_NEchoImages);
  ...
  if(PVM_SignalType == SignalType_Fid)
  {
    FirstGradientEchoTime = MAX_OF(FirstGradientEchoTime, PVM_MinEchoTime);
  }
  else
  {
    SpinEchoTime = MAX_OF(SpinEchoTime,  PVM_MinEchoTime);
    FirstGradientEchoTime = MAX_OF(FirstGradientEchoTime, 0.0);
  }

  /* Set Echo Parameters for Scan Editor   */
  /* echo spacing */
  PVM_EchoTime = 1000 * PVM_Matrix[0] / (PVM_EffSWh * PVM_AntiAlias[0]);
  PVM_EchoPosition = 50.0;

  ParxRelsShowInEditor("EffectiveEchoTime");
  ParxRelsMakeNonEditable("EffectiveEchoTime");
  ParxRelsShowInEditor("GradientEchoTime");
  ParxRelsMakeNonEditable("GradientEchoTime");

  DB_MSG(("Exiting T2S_EPI/backbone.c:echoTimeRels"));
}
...
void repetitionTimeRels( void )
{
  ...
  multiecho_time=(PVM_NEchoImages-1)*EchoTimeIncrement;
  ...
  PVM_MinRepetitionTime =
    ...
    nSlices * (0.030 +       //UpdateDynPars
               ...
               (PVM_SignalType == SignalType_Fid ? 0.0 : SpinEchoTime) +
