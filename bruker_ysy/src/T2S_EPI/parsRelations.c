...
/* -----------------------------------------------------------
   Relation of PVM_NEchoImages
 
   -----------------------------------------------------------*/
void NEchoRelation(void)
{
  DB_MSG(("-->NEchoRelation"));

  /* Tell the request handling system that the parameter
     PVM_NEchoImages has been edited */
  UT_SetRequest("PVM_NEchoImages");

  /* Check the values of PVM_NEchoImages */
  NEchoRange();
...
/* --------------------------------------------------------------
   Range checking of PVM_NEchoImages

   --------------------------------------------------------------*/
void NEchoRange(void)
{
  DB_MSG(("-->NEchoRange"));
  ...
  PVM_NEchoImages=MAX_OF(PVM_NEchoImages,1);
  PVM_NEchoImages=MIN_OF(PVM_NEchoImages,32);
...
/* -----------------------------------------------------------
   Relation of EchoTimeIncrement
 
   -----------------------------------------------------------*/
void EchoTimeIncrementRelation(void)
{
  DB_MSG(("-->EchoTimeRelation"));

  /* Tell the request handling system that the parameter
     EchoTimeIncrement has been edited */
  UT_SetRequest("EchoTimeIncrement");

  /* Check the values of EchoTimeIncrement */
  EchoTimeIncrementRange();

  /* call the backbone; further handling will take place there */
 
  backbone();

  DB_MSG(("<--EchoTimeIncrementRelation"));
}

/* --------------------------------------------------------------
   Range checking of EchoTimeIncrement

   --------------------------------------------------------------*/
void EchoTimeIncrementRange(void)
{
  DB_MSG(("-->EchoTimeRange"));
  
  /* allowed clasification */

  if(ParxRelsParHasValue("EchoTimeIncrement") == No)
    EchoTimeIncrement = 20.0;
  
  EchoTimeIncrement = MAX_OF(EchoTimeIncrement,0.100);
  EchoTimeIncrement=MIN_OF(EchoTimeIncrement,1000.0);

  DB_MSG(("<--EchoTimeIncrementRange"));

}

/* -----------------------------------------------------------
   Relation SinglePhaseCorrectionRelation;
 
   -----------------------------------------------------------*/
void SinglePhaseCorrectionRelation(void)
{
  DB_MSG(("-->SinglePhaseCorrectionRelation"));

  /* Tell the request handling system that the yes/no parameter
     SinglePhaseCorrectionRelation has been edited */
  UT_SetRequest("SinglePhaseCorrection");
