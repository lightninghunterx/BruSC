...
void backbone(void)
{
  ...
  MRT_UpdateDigPars(&PVM_EffSWh, PVM_EncMatrix[0] * PVM_EncMatrix[1], NULL, NULL);
...
void UpdateGeometry(void)
{
  ...
  double minSliceThick = MRT_MinSliceThickness(
                           ExcPul.Bandwidth,
                           0,
                           SliceGradLim,
                           0,
...
void UpdateEchoTime(void)
{
  ...
  {
    PVM_MinEchoTime += PVM_SpiralEchoTime;

    SetEchoTime();

    TEWait    = PVM_EchoTime - PVM_MinEchoTime;
    TEWaitRfc = 0.0;
  }
  else
  {
    ...
    PVM_MinEchoTime = 2.0 * MAX_OF(minTE1, minTE2);

    SetEchoTime();

    TEWait    = PVM_EchoTime / 2.0 - minTE1;
    TEWaitRfc = PVM_EchoTime / 2.0 - minTE2;
  }

  /* prolong rephasing if possible for smaller gradient */

  double rephGradDurExt = PTB_GetSpatDim() > 2 ? MRT_MaxEncodingTime(PVM_Fov[2], PVM_GradCalConst) - RephGradDur : 10.0;
  rephGradDurExt = MAX_OF(0.0, MIN_OF(TEWait, rephGradDurExt));
  RephGradDur += rephGradDurExt;
  TEWait -= rephGradDurExt;
...
void SetEchoTime(void)
{
  DB_MSG(("-->SetEchoTime"));

  const double minEchoTimePlusDelay = PVM_MinEchoTime + MinDelay;

  if (MinimiseTE == Yes)
  {
    PVM_EchoTime = minEchoTimePlusDelay;
    ParxRelsMakeNonEditable("PVM_EchoTime");
  }
  else
  {
    PVM_EchoTime = MAX_OF(minEchoTimePlusDelay, PVM_EchoTime);
...
void UpdateRepetitionTime(void)
{
  ...
  PVM_MinRepetitionTime *= (double)PVM_NMovieFrames;
  ...
  PVM_ScanTime = PVM_ScanTime * PVM_SatTransRepetitions *PVM_NRepetitions;
...
void UpdateSliceGradients()
{
  ...
  {
    RfcSpoilGrad = MAX_OF(30.0, MIN_OF(SliceGradLim, 2.0 * SliceGrad));
    RfcSpoilDur = MAX_OF(RefPul.Length, 2.0 * RiseT);
  }
  else
  {
    RfcSpoilGrad = 0.0;
...
void UpdateRFPulses(void)
{
  ...
  /* update with matched bandwidth
     (may be replaced once toolbox function with this functionality is available) */
  if (PVM_SignalType == SignalType_Fid)
  {
  ...
  {
    if (UT_GetRequest("RefPul") == Yes)
    {
      UpdateRFPulsesWithBandwidthMatch("RefPul", &RefPul, "ExcPul", &ExcPul);
    }
    else
    {
      UpdateRFPulsesWithBandwidthMatch("ExcPul", &ExcPul, "RefPul", &RefPul);
    }
  }
  ...
  if (PVM_SignalType == SignalType_Fid)
  {
    ParxRelsHideInEditor("RefPul, RefPulEnum, RefPulAmpl");
  }
  else
  {
    ParxRelsShowInEditor("RefPul, RefPulEnum");
    if (PVM_DeriveGains == Yes)
    {
      ParxRelsHideInEditor("RefPulAmpl");
    }
    else
    {
      ParxRelsShowInEditor("RefPulAmpl");
    }
  }
...
void UpdateRFPulsesWithBandwidthMatch(
  const char   *pulseName1,
  PVM_RF_PULSE *pulse1,
  const char   *pulseName2,
  PVM_RF_PULSE *pulse2)
{
  DB_MSG(("<--UpdateRFPulsesWithBandwidthMatch"));

  /* update master */
  STB_UpdateRFPulse(pulseName1, 1, PVM_DeriveGains);

  /* update slave with matched bandwidth */
  pulse2->Bandwidth = pulse1->Bandwidth;
  STB_UpdateRFPulse(pulseName2, 1, PVM_DeriveGains);

  /* adapt master to slave if required */
  if (pulse2->Bandwidth != pulse1->Bandwidth)
  {
    pulse1->Bandwidth = pulse2->Bandwidth;
    UT_ClearRequest();
    STB_UpdateRFPulse(pulseName1, 1, PVM_DeriveGains);
  }
