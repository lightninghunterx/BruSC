...
const char * PRESS_optHandler_id = "$Id$ (C) 2002 - 2015 Bruker BioSpin MRI GmbH";
...
void OptHandler ( void )
{
  ...
  {
    double min,integ1,integ2,ratio;
    ...
    OPT_GradReadDephaseDelay = 0.5*riseT+0.02+PVM_AcquisitionTime/2.0;

    integ1=OPT_GradReadDephaseDelay; //effective duration of read dephase
    integ2=0.5*riseT+0.02+PVM_AcquisitionTime/2.0; //effective duration of read grad till echo
    ratio = integ2 / integ1;
    ...
    min = MRT_MinReadFov(PVM_EffSWh,
                         ...
                         100.0,
                         PVM_GradCalConst);
    OPT_FOV = MAX_OF( min, OPT_FOV);

    OPT_GradStrengthRead = MRT_ReadGrad(PVM_EffSWh,OPT_FOV,PVM_GradCalConst);
    OPT_GradStrengthReadDephase = MRT_ReadDephaseGrad(ratio, OPT_GradStrengthRead);
...
void HandleAdjustmentRequests(void)
{
  ...
  if(!strcmp(adjName, "AutoLocShim"))
  {
      ...
      if(PTB_AdjHandlerCall(&mode) &&
          mode==HANDLE_PLATFORM) // setting TR=0 once when adjplatform is entered, but again when adj is started from there
      {
      ...
      /* Use calculated frequency as adj result. Store PVM_AdjShimset to avoid re-adjustment of global Frequ by ScanShim */
  ...
  else if(Yes==PTB_AdjMethSpec() && 0 == strcmp(adjName, RG_ADJNAME))
  {
      PVM_DummyScans=1;
      ParxRelsParRelations("PVM_DummyScans",No);
      PVM_NAverages=1;
...
void AdjustSF( void )
{
  ...
  */
  ...
  else
  {
    ...
    oerr << "Frequency Adjustment (PRESS) failed due to small Peak/Noise ratio"
         << std::endl << errmsg.str();
