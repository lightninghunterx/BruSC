...
void SetBasicParameters( void )
{
  ...
  PARX_change_dims("ACQ_obj_order",nSlices);
...
void SetInfoParameters( void )
{
  ...
  PARX_change_dims("ACQ_time_points",1);
  ACQ_time_points[0] = 0;
...
void SetAcquisitionParameters(void)
{
  ...
  int dim      = PTB_GetSpatDim();
  int nSlices  = GTB_NumberOfSlices( PVM_NSPacks, PVM_SPackArrNSlices );
  int transAct = AngioMode == Yes ? 1 : nSlices;
  int nTotal   = PVM_EncMatrix[1] * Oversampling * nSlices * NA * NR;
  if(dim>2)
    nTotal = nTotal*PVM_EncMatrix[2];

  navigator = ATB_SetNavBaseLevel(jobParameters, 5, transAct, nTotal, 0);
...
void printTiming(void)
{
  ...
  double rampT = CFG_GradientRampTime()+CFG_InterGradientWaitTime();
  double del;

  if(PVM_NavPosMode != Nav_in_slice)
    del = d(10)+rampT;
  else
    del = d(14)+d(3)+PVM_NavAQ+0.1+d(10)+rampT;
  
  /* TE */
  te = p(0)/2+rampT+del+d(3)+aqq/2 + TeFillDelay;

  DB_MSG(("te: %f should be %f diff %f\n",
	  te,PVM_EchoTime,te-PVM_EchoTime));

  /* TR */
  tr = 0.03+0.04+d(6)+d(3)+p(0)/2+aqq/2+d(11)+d(12)+d(3)+d(0)+te+PVM_NavigatorModuleTime;
  if(AngioMode==No)
    tr *= NSLICES;
