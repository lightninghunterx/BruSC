...
const char * RARE_BaseLevelRelations_id = "$Id$(C) 2002-2014 Bruker BioSpin MRI GmbH";
...
void SetBasicParameters( void )
{
  ...
  ATB_SetNI( nSlices * PVM_NEchoImages * PVM_FwsLoop);
  ...
  ATB_SetAcqObjOrder( nSlices, PVM_ObjOrderList, PVM_NEchoImages, PVM_FwsLoop );
...
void SetPpgParameters( void )
{
  ...
  L[2] = PVM_EncGenTotalSteps/PVM_RareFactor;
...
void SetAcquisitionParameters(void)
{
  ...
  int nTotalScansJob1 = NR * NAE * NA * PVM_FwsLoop;

  nTotalScansJob1 *= PVM_EncGenTotalSteps;
  nTotalScansJob1 /= PVM_RareFactor;

  ATB_SetDriftCompBaseLevel(jobPars, NA, nTotalScansJob1, PVM_DummyScans * PVM_FwsLoop);
...
void printTiming(void)
{
  ...
  /* TR */
  double echo_loop_time = PVM_RareFactor * PVM_NEchoImages * (p(1) + aqq + 2*d(3) + 2*d(5) + 2*d(6));

  tr = 0.03 + 0.01 + d(9) + d(3) + p(0) + d(2) + d(1) + echo_loop_time + d(3) + d(0);
  ...
  tr *= NSLICES;
  tr += PVM_DriftCompModuleTime + PVM_BlBloodModuleTime + PVM_SelIrModuleTime;

  if (PVM_TriggerMode == per_PhaseStep)
    tr += (PVM_BlBloodOnOff==On)? 2*PVM_TriggerModuleTime : PVM_TriggerModuleTime;
  else if (PVM_TriggerMode == per_Slice)
    tr += NSLICES * PVM_TriggerModuleTime;
