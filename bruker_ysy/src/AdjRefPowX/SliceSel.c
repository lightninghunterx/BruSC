/* ***************************************************************
 *
 * $Source: /pv/CvsTree/pv/gen/src/prg/methods/AdjRefPowX/SliceSel.c,v $
 *
 * Copyright (c) 2020
 ...
 try
  {
    ...
    double effPulseDur = 1e-3*ExcDurList[0] * (ExcPulse.Rpfac/100);
    ...
    else
    {
      ...
      // if slicesel 'On' restrict scan mode to vary_length.
      // Otherwise gradient adaptions are required
      if ( ScanMode == CONST_PULSE_POW )
        ScanMode = CONST_PULSE_DUR;
      ...
      double pbw       = 1e3*ExcPulse.Bwfac/ExcDurList[0]; //1e3 as durlst has us
